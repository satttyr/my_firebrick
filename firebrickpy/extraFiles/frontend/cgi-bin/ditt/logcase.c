...
redistribution.
******************************************************************************/
static char *SCCS_ID[] = {"@(#) logcase.c Linux Version 1.2 Created 02/18/05 at 08:49:40",
				__DATE__,__TIME__};
/***** Author: Dr. James R. Lyle, NIST/SDCT/SQG ****/
/* Modified by Kelsey Rider, NIST/SDCT May 2004 */
# include <stdio.h>
# include "zbios.h"
# include <time.h>
# include <string.h>
...
void print_help(char *p)
{
	...
	printf ("Usage: %s test-case host operator src dst [media]\n",p);
}
...
*****************************************************************/
	...
	fprintf (log,"Disks: src(%s) dst (%s) other (%s)\n",p[4],p[5],
		((np >= 7) && (strlen(p[6]) > 0))?p[6]:"none");
	fprintf (log,"Date: %s\n",ctime(&from));
